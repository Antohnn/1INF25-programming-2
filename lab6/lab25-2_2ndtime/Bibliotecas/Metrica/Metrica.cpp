...
ifstream& Metrica::leer(ifstream &arch) {
    ...
    char descripcionMetrica[200],car;

    if (arch>>idMetrica) {
        arch.get();
        arch.getline(descripcionMetrica,200,',');
...
ofstream& Metrica::imprimir(ofstream &archRep) {
    archRep<<setfill('0')<<setw(4)<<this->get_id()<<setfill(' ')<<" "<<this->get_fecha_calculo()
