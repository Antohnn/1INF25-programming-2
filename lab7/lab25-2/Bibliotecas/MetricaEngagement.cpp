...
ifstream& MetricaEngagement::leer(ifstream &arch) {
    ...
    int msjChat,userChat,clipGen;
    char car;
    arch>>msjChat>>car>>userChat>>car>>clipGen;
    this->set_mensajes_chat(msjChat);
    this->set_usuarios_unicos_chat(userChat);
    this->set_clips_generados(clipGen);
...
ofstream& MetricaEngagement::imprimir(ofstream &archRep) {
    ...
    archRep<<this->get_mensajes_chat()<<" "<<this->get_mensajes_chat()<<" "<<this->get_clips_generados()<<" ";
    imprimirDescripcion(archRep);
    return archRep;
}
//
TipoMetrica MetricaEngagement::getTipo() const {
    return METRICA_ENGAGE;
}
//
